...
To prevent data conflicts when a coach opens the same athlete on multiple devices, implement edit locking.

### Data Model Addition

Add `edit_lock` field to athlete documents:

```python
# models/athlete.py
class EditLock(BaseModel):
    locked_by: str          # user.id
    locked_at: datetime     # timestamp when lock acquired
    expires_at: datetime    # auto-release after 5 minutes
...
|--------|----------|-------------|
| POST | `/athletes/{id}/lock` | Acquire edit lock |
| DELETE | `/athletes/{id}/lock` | Release edit lock |
| GET | `/athletes/{id}` | Returns lock status in response |
...
@router.post("/{athlete_id}/lock")
async def acquire_lock(
    athlete_id: str,
    user: User = Depends(get_current_user)
):
    """Acquire edit lock on athlete"""
    athlete_repo = AthleteRepository()
    athlete = await athlete_repo.get_if_owned(athlete_id, user.id)
    ...
    if athlete.edit_lock:
        if athlete.edit_lock.expires_at > datetime.utcnow():
            if athlete.edit_lock.locked_by != user.id:
                raise HTTPException(
                    status_code=409,
                    detail="Athlete is being edited in another session"
                )
        # Lock expired or same user - allow re-lock
    ...
    await athlete_repo.update(athlete_id, lock)

    return {"message": "Lock acquired", "expires_in_seconds": LOCK_DURATION_MINUTES * 60}

@router.delete("/{athlete_id}/lock")
async def release_lock(
    athlete_id: str,
    user: User = Depends(get_current_user)
):
    """Release edit lock on athlete"""
    athlete_repo = AthleteRepository()
    athlete = await athlete_repo.get_if_owned(athlete_id, user.id)
    ...
    if athlete.edit_lock and athlete.edit_lock.locked_by != user.id:
        raise HTTPException(status_code=403, detail="Not lock owner")

    await athlete_repo.update(athlete_id, {"edit_lock": None})
...
1. When coach opens athlete profile for editing, frontend calls `POST /athletes/{id}/lock`
2. If locked by another session, show read-only view with message: "This athlete is being edited in another session"
3. Lock auto-expires after 5 minutes of inactivity
4. Lock released when coach navigates away or explicitly saves
5. Frontend should refresh lock every 4 minutes if still editing
